      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <div className="mt-12 grid gap-8 md:grid-cols-2 xl:grid-cols-3">
          {data.items.map((project) => (
            <article
              key={project.title}
              className="flex h-full flex-col rounded-3xl border border-rose-100 bg-white/85 p-6 shadow-sm transition-transform transition-colors duration-500 hover:-translate-y-1 hover:shadow-lg dark:border-rose-800 dark:bg-black/60"
            >
              {project.link ? (
                <a
                  href={project.link}
