...
const ExperienceHighlights = ({ data }: ExperienceHighlightsProps) => (
  ...
  >
    <div className="pointer-events-none absolute left-12 top-8 h-64 w-64 rounded-full bg-rose-200/40 blur-3xl dark:hidden" aria-hidden />
      ...
      <div className="grid gap-8 md:grid-cols-2">
        {data.items.map((item) => (
          <article
            ...
            className="rounded-3xl border border-rose-100 bg-white/85 p-8 shadow-sm transition-transform hover:-translate-y-1 hover:shadow-lg dark:border-neutral-800 dark:bg-neutral-900/80"
          >
            <div className="flex flex-col gap-1 text-left">
              ...
              <p className="text-sm font-medium text-gray-600 dark:text-neutral-300">{item.company}</p>
            </div>
            <p className="mt-4 text-sm leading-relaxed text-gray-600 dark:text-neutral-300">{item.description}</p>
            <ul className="mt-6 space-y-2 text-sm text-gray-600 dark:text-neutral-200">
